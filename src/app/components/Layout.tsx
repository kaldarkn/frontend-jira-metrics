...
const metrics = [
  { name: 'Task Time Discrepancies In Groups', icon: <Speed /> },
  { name: 'Task Time Discrepancies In Projects', icon: <Speed /> },
  { name: 'User Task Time Discrepancies', icon: <Person /> },
];
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <Drawer
        ...
        <List>
          {metrics.map((metric) => (
            <CustomLink key={metric.name} to={metric.name.replace(/\s/g, '')}>
