...
import { JiraProjectType } from '../../redux/slices/projects/slice';
import { FETCH_PROJECTS } from '../../sagas/projects/actions';
import { FETCH_USER_ISSUES_BY_PROJECT } from '../../sagas/userIssues/actions';
...
const TaskTimeDiscrepanciesInProjects = () => {
  const [requestParams, setRequestParams] = useState({ projectId: '', startDate: '', endDate: '' });
  const [project, setProject] = useState<JiraProjectType | null>(null);
  const [calculateData, setCalculateData] = useState<TaskTimeDiscrepanciesType[]>([]);
  const { projects, userIssues } = useSelector((state: RootState) => state);
  ...
  useEffect(() => {
    project &&
      setRequestParams((prevRequestParams) => ({ ...prevRequestParams, projectId: project.id }));
  }, [project]);
  ...
  return (
    <Stack flexDirection="column" alignItems="center">
      ...
      {projects.error && <ErrorMessage error={projects.error} visible={!!projects.error} />}
      <Typography variant="h3" component="h1" marginBottom={8}>
        Task time discrepancies in projects
      </Typography>
      <Stack flexDirection="row" justifyContent="center" alignItems="center">
        <DemoContainer components={['DatePicker', 'DatePicker']}>
          <Autocomplete
            isOptionEqualToValue={(option: JiraProjectType, value: JiraProjectType) =>
              option.id === value.id
            }
            value={project}
...
export default TaskTimeDiscrepanciesInProjects;
